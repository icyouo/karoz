...
5. Karoz 检测代码变化、按需执行验证命令、在任务分支提交，并合并进你的本地 base 分支——push 留给你决定。
...
- 扫描并管理本地项目
- 项目级常驻 Agent，带消息与记忆
- 多 Agent 团队：基于角色的交接与共享黑板
- 交接状态机 + 每 Agent 串行、可恢复队列
- 本地 Skill 发现、列举与 `$提及` 注入
- MCP 工具支持，stdio 与 SSE，项目级或全局
- 开发和部署任务记录
- 实时任务状态与流式运行日志
- `codex` 和 `claude` CLI 诊断
- 常驻聊天 Agent：Codex OAuth 直连或本地 CLI
- 任务执行委托给原生 `codex`/`claude` 编码 Agent
