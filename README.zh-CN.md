...
Karoz 闭合了这个回路。每个项目拥有一个带独立记忆的常驻 Agent 和一套可追踪的任务工作流。你在浏览器里描述目标，让 Agent 结合真实项目上下文工作；开发任务在隔离的 worktree 中运行——实时日志、差异、提交，全部由你审查后再合入主分支。上下文留在项目里，代码留在你的机器上，每一步都可见、可中断。
...
```
  ...
  任务「加分页」  →  隔离 git worktree  →  Agent 改动
                 →  差异检测  →  可选验证
                 →  提交  →  你审查  →  合并
```
...
- **从建议走到交付。** Karoz 不止于生成代码。它在隔离环境中执行、检测改动、按需验证，并产出一个可供你审查的 Git 提交。
...
2. 项目级 `karoz` Agent 跨会话持续保留消息、记忆和上下文。
3. 你创建开发或部署任务，并在浏览器里实时跟踪状态与日志。
4. 开发任务在隔离的 Git worktree 中运行——绝不直接改动你的主工作区。
5. Karoz 检测代码变化，按需执行验证命令，然后提交结果供你审查和合并。
...
Karoz 目前处于 MVP 阶段。当前重心是验证核心回路——项目级 Agent + 可追踪任务执行。界面、扩展机制和团队协作能力仍在快速演进。
