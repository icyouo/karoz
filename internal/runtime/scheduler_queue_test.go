...
func TestSchedulerQueueSerialDedupAndRetry(t *testing.T) {
	...
	if claimed.ID != first.ID {
		t.Fatalf("later job passed the retried earlier job, claim=%+v", claimed)
	}
	queue.Complete(first.ID, CompletionSucceeded, "", now.Add(5*time.Second))
	claimed, _ = queue.Claim(key, now.Add(6*time.Second))
	if claimed.ID != second.ID {
		t.Fatalf("later queued job = %+v", claimed)
	}
	queue.Complete(second.ID, CompletionSucceeded, "", now.Add(7*time.Second))

	exhaustedJob := ScheduledRun{ID: "exhausted", ProjectID: "p1", AgentID: "designer", Kind: ScheduledHandoff, Status: ScheduledQueued, DedupKey: "handoff/exhausted", MaxAttempts: 1, CreatedAt: now.Add(8 * time.Second)}
	queue.Enqueue(exhaustedJob)
	queue.Claim(key, now.Add(9*time.Second))
	exhausted := queue.Complete(exhaustedJob.ID, CompletionFailed, "permanent", now.Add(10*time.Second))
	if exhausted.Requeue || !exhausted.NotifyFailure || exhausted.Job.Status != ScheduledFailed || queue.HasDedup(exhaustedJob.DedupKey) {
