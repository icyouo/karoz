...
func (a *app) residentStaticToolHandlers() map[string]tooldomain.Handler[ResidentToolContext] {
	return map[string]tooldomain.Handler[ResidentToolContext]{
...
func initializeResidentToolRegistry(
	specs []map[string]any,
	handlers map[string]tooldomain.Handler[ResidentToolContext],
	policies residentToolPolicyRegistry,
) (*tooldomain.Registry[ResidentToolContext], error) {
	handlerNames := make(map[string]bool, len(handlers))
	for name := range handlers {
		handlerNames[name] = true
	}
	if err := validateResidentToolPolicyBijection(specs, handlerNames, policies); err != nil {
		return nil, err
	}
	registry := tooldomain.NewRegistry[ResidentToolContext]()
	for _, spec := range specs {
		definition := definitionFromResidentToolSpec(spec)
		if err := registry.Register(definition, handlers[definition.Name]); err != nil {
			return nil, err
		}
	}
	return registry, nil
...
func (a *app) initializeResidentToolRegistry() error {
	registry, err := initializeResidentToolRegistry(
		residentStaticToolSpecs(),
		a.residentStaticToolHandlers(),
		staticResidentToolPolicyRegistry,
	)
	if err != nil {
		return err
	}
...
func (a *app) executeResidentTool(ctx context.Context, toolCtx ResidentToolContext, call codexToolCall) (string, error) {
	...
	if toolCtx.EnforcePolicy && !a.residentToolAuthorized(toolCtx, call.Name) {
	...
	if registry == nil {
		return toolJSON(map[string]any{"error": "tool_registry_unavailable", "message": "static resident tools are not initialized"}), nil
	}
	result, found, err := registry.Execute(ctx, toolCtx, call.Name, args)
	if !found {
