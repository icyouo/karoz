...
func (a *app) executeHandoffScheduledRun(ctx context.Context, job ScheduledRun) error {
	...
	userText := strings.TrimSpace(fmt.Sprintf("You received an asynchronous %s from agent %s.\n\nInbox message id: %s\nSubject: %s\nObjective: %s\nExpected output: %s\n\n%s\n\nRespond as %s. %s A reply is terminal and must never receive another reply; additional work requires a new send_to handoff. Send decisions/conflicts to Karoz only as reports; otherwise avoid forwarding unless another agent is clearly required. If this handoff requires tracked coding or deployment work, create a task.", msg.Intent, msg.SourceAgentID, msg.ID, msg.Subject, msg.Objective, msg.ExpectedOutput, msg.Body, firstNonEmpty(target.Nickname, target.DisplayName, target.Name, target.ID), closeInstruction))
